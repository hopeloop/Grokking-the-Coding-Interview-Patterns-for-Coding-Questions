...
def search_quadruplets(nums, target):
    ...
    for fir in range(len(nums) - 3):
        if fir != 0 and nums[fir] == nums[fir - 1]:
            continue
        for sec in range(fir + 1, len(nums) - 2):
            if sec > fir + 1 and nums[sec] == nums[sec - 1]:
                continue
            left, right = sec + 1, len(nums) - 1
            while left < right:
                cur_sum = nums[left] + nums[right] + nums[fir] + nums[sec]
                if cur_sum == target:
                    quadruplets.append([nums[fir], nums[sec], nums[left], nums[right]])
                    left += 1
                    right -= 1
...
def fourSum(nums, target):
    ...
    for i in range(length - 3):
        if i > 0 and nums[i] == nums[i - 1]:
            continue
        # 从i个元素开始，无论如何组合后面的元素都大于target的情况可以直接跳出循环
        if nums[i] + nums[i + 1] + nums[i + 2] + nums[i + 3] > target:
            break
        # 当第i个数与最大的三个数相加仍小于 target 时，可以跳过这个i
